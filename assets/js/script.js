		...
		if(data.length){
			data.forEach(function (item) {
				if(item.id == index){
					...
					//container.getElementByClassName('sentence').text(item.sentence);
					container.find('p.description').text(item.description);
					container.find('p.sentence').text(item.sentence);
					//container.find('sentence').text(item.sentence);
				}
			});
		}
